...
class DepthVisionTransformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        # Lưu skip connection từ đầu ra của patch embedding (THÊM MỚI: lưu skip connection)
        x_skip = x.clone()  # [B, num_patches, embed_dim]
        ...
        # Reshape skip connection từ patch embedding (THÊM MỚI: reshape skip connection)
        x_skip = x_skip.transpose(1, 2).reshape(B, self.embed_dim, H_patch, W_patch)  # New: reshape skip connection
        
        # 5. Kết hợp skip connection: giữ lại thông tin chi tiết từ đầu vào (THÊM MỚI: kết hợp skip connection)
        x = x + x_skip  # New: kết hợp skip connection
        
        # 6. Dự đoán depth map: output shape [B, 1, 384, 384]
